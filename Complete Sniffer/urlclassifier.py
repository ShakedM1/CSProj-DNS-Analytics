...
df = pd.read_csv(r"D:\Downloads\URL-Classification-master\dataset.csv")
#df=df.sample(frac=1)
...
print(str(len(Suspicious_TLD)))
print(str(len(Suspicious_Domain)))
...
def checkurl(address):
 ...
 'presence of double slash','no of subdir','no of subdomain','len of domain','no of queries','is IP','presence of Suspicious_TLD',\
 'presence of suspicious domain','label'))
  results = getFeatures(address, '1')
  result.loc[0] = results
  result = result.drop(['url','label'],axis=1).values
  str = clf.predict(result)
  print(str)
  if str == '1':
       return "malicious"
...
results = getFeatures('	https://mirika.myscriptcase.com/mirika.html', '1')
result.loc[0] = results
result = result.drop(['url','label'],axis=1).values
...
with open('C:\Users\shaked\Desktop\complete_model.sav', 'wb') as f:
    pickle.dump(clf, f)
